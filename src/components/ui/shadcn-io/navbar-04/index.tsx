...
import { SearchIcon, LogOut, User as UserIcon } from 'lucide-react'; // Tambah icon logout & user
...
const HamburgerIcon = ({ className, ...props }: React.SVGAttributes<SVGElement>) => (
  <svg className={cn('pointer-events-none', className)} width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" xmlns="http://www.w3.org/2000/svg" {...(props as any)}>
...
export interface Navbar04Props extends React.HTMLAttributes<HTMLElement> {
  ...
  user?: User | null; // TAMBAHAN: Menerima prop user
}
  ...
  (
    {
      ...
      onCartClick,
      onSearchSubmit,
      user, // Gunakan prop user di sini
      ...props
    },
    ref
  ) => {
    ...
    // HAPUS useEffect getUser, karena kita mengandalkan prop user dari layout
    // HAPUS state internal user, gunakan prop
    ...
    const handleSearchSubmit = (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (onSearchSubmit) onSearchSubmit(formData.get('search') as string);
    };
        ...
        <div className="container mx-auto flex h-16 max-w-screen-2xl items-center justify-between gap-4">
          <div className="flex flex-1 items-center gap-2">
                ...
                <PopoverContent align="start" className="w-64 p-4">
                  <div className="flex flex-col gap-2">
                    {navigationLinks.map((link, index) => (
                      <Button key={index} variant="ghost" asChild className="w-full justify-start text-left font-medium h-10 px-3">
                        <Link href={link.href || '#'} onClick={() => setIsPopoverOpen(false)}>
                          {link.label}
                        </Link>
                      </Button>
                    ))}
                    ...
                    {!user ? (
                      <Button variant="ghost" asChild className="w-full justify-start text-left h-10 px-3">
                        <Link href={signInHref || '#'} onClick={() => setIsPopoverOpen(false)}>
                          {signInText}
                        </Link>
                      </Button>
                    ) : (
                      <>
                        <Button variant="ghost" asChild className="w-full justify-start text-left h-10 px-3">
                          <Link href={cartHref || '#'} onClick={() => setIsPopoverOpen(false)}>
            ...
            <div className="flex flex-1 items-center gap-6 max-md:justify-between">
              ...
              <form onSubmit={handleSearchSubmit} className="relative">
                <Input id={searchId} name="search" className="peer h-8 ps-8 pe-2" placeholder={searchPlaceholder} type="search" />
          ...
          {!isMobile && (
            <div className="flex items-center gap-3">
              {!user ? (
                <Button variant="ghost" size="sm" asChild className="text-sm font-medium hover:bg-accent hover:text-accent-foreground cursor-pointer">
                  <Link href={signInHref || '#'} onClick={(e) => { if (onSignInClick) onSignInClick(); }}>
                    {signInText}
                  </Link>
                </Button>
              ) : (
                <>
                  <Button variant="ghost" size="sm" asChild className="text-sm font-medium px-4 h-9 rounded-md cursor-pointer">
                    <Link href={cartHref || '#'} onClick={(e) => { if (onCartClick) onCartClick(); }}>
