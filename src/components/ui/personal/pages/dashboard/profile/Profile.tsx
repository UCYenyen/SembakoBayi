...
export default function Profile({ addresses }: { addresses: Address[] }) {
    ...
    const {
        formData, displayPhone, handlePhoneChange, handleChange,
        handleSendVerification, handleVerifyOtp, handleSave,
        errors, isLoading, isOtpDialogOpen, setIsOtpDialogOpen,
        otpCode, setOtpCode
    } = useProfile({
                ...
                <CardContent className='space-y-8 py-6'>
                    <div className='grid gap-6'>
                        <div>
                            ...
                            <p className='text-sm text-muted-foreground'>Perbarui detail foto dan informasi pribadi Anda.</p>
                        </div>
                                ...
                                <Select>
                                    ...
                                    <SelectContent>
                                        {addresses.map(address => (
                                            <SelectItem key={address.id} value={address.id}>{address.label}</SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                                <Button variant="secondary" className="gap-2"><Plus className="w-4 h-4" /> Baru</Button>
                            </div>
                        </div>
                    </div>
                </CardContent>
